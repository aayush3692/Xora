...
const TestimonialItem = ({ item, containerClassName }) => {
    return (
        <div className={clsx(
            "relative px-14 pt-11 pb-14 after:absolute after:bottom-0 after:right-0 after:h-0.5 after:w-screen after:bg-s2 after:content-[''] max-md:px-0 max-md:pt-11 max-md:after:-right-4",
            containerClassName)}>

            <blockquote className="h6 mb-8 text-p4">{item.comment}</blockquote>

            <div className='flex items-center max-xl:-mr-8'>
                <div className='mr-4 size-40 shrink-0 rounded-half border-2 border-s2 p-1.5'>
                    <img
                        src={item.avatarUrl}
                        alt={item.name}
